...
function attachSolutionButtonListeners_question20(button) {
  button.addEventListener('click', function() {
    ...
    const selectedAnswers = Array.from(document.querySelectorAll('input[name="rpfCheck"]:checked')).map(input => input.value);

    // Show correct/incorrect styling for each option
    document.querySelectorAll('input[name="rpfCheck"]').forEach(input => {

...
function attachSolutionButtonListeners_question13(button) {
  button.addEventListener('click', function() {
    ...
    const selectedAnswers = Array.from(document.querySelectorAll('input[name="sslActions"]:checked'))
      .map(input => input.value);

    // Show correct/incorrect styling for each option
    document.querySelectorAll('input[name="sslActions"]').forEach(input => {
    ...
    if (solutionInfoElement) {
      ...
      if (allCorrect) {
        ...
        if (yourAnswerElement) {
          document.getElementById('yourAnswerOptionA_13').textContent = `A. Allow & Warning: ${selectedAnswers.includes('A') ? 'Selected' : 'Not Selected'}`;
          document.getElementById('yourAnswerOptionB_13').textContent = `B. Trust & Allow: ${selectedAnswers.includes('B') ? 'Selected' : 'Not Selected'}`;
          document.getElementById('yourAnswerOptionC_13').textContent = `C. Allow: ${selectedAnswers.includes('C') ? 'Selected' : 'Not Selected'}`;
          document.getElementById('yourAnswerOptionD_13').textContent = `D. Block & Warning: ${selectedAnswers.includes('D') ? 'Selected' : 'Not Selected'}`;
          document.getElementById('yourAnswerOptionE_13').textContent = `E. Block: ${selectedAnswers.includes('E') ? 'Selected' : 'Not Selected'}`;

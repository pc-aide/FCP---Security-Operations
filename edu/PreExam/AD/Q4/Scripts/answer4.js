...
function attachSolutionButtonListeners_question4(button) {
  button.addEventListener('click', function() {
    ...
    const selectedAnswers = Array.from(document.querySelectorAll('input[name="fgcpProtocol"]:checked')).map(input => input.value);

    // Show correct/incorrect styling for each option
    document.querySelectorAll('input[name="fgcpProtocol"]').forEach(input => {

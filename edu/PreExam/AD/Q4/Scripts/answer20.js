// Function to attach event listeners to the solution button for the new question
function attachSolutionButtonListeners_question20(button) {
  button.addEventListener('click', function() {
    ...
    const selectedAnswers = Array.from(document.querySelectorAll('input[name="fortiGateRequirements"]:checked')).map(input => input.value);

    // Show correct/incorrect styling for each option
    document.querySelectorAll('input[name="fortiGateRequirements"]').forEach(input => {

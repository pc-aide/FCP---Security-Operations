...
function attachSolutionButtonListeners_question10(button) {
  button.addEventListener('click', function() {
    ...
    const selectedAnswer = document.querySelector('input[name="appControlEngine"]:checked')?.value;

    // Show correct/incorrect styling for each option
    document.querySelectorAll('input[name="appControlEngine"]').forEach(input => {

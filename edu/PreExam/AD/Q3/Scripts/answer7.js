...
function attachSolutionButtonListeners_question7(button) {
  button.addEventListener('click', function() {
    ...
    const selectedAnswers = Array.from(document.querySelectorAll('input[name="denyAccess"]:checked')).map(input => input.value);

    // Show correct/incorrect styling for each option
    document.querySelectorAll('input[name="denyAccess"]').forEach(input => {

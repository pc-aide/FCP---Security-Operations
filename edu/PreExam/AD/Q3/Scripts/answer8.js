...
function attachSolutionButtonListeners_question8(button) {
  button.addEventListener('click', function() {
    ...
    const selectedAnswers = Array.from(document.querySelectorAll('input[name="antivirusMode"]:checked')).map(input => input.value);

    // Show correct/incorrect styling for each option
    document.querySelectorAll('input[name="antivirusMode"]').forEach(input => {
